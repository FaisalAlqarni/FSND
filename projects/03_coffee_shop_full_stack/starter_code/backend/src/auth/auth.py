...
'''
@TODOx implement get_token_auth_header() method
...
def get_token_auth_header():
    ...
    return token
'''
@TODOX implement check_permissions(permission, payload) method
    @INPUTS
...
def check_permissions(permission, payload):
    ...
    if permission not in payload:
        raise AuthError({
            'code': 'invalid_claims',
            'description': 'permission key not included',
        }, 400)  
...
'''
@TODOX implement verify_decode_jwt(token) method
    @INPUTS
...
'''
@TODOX implement @requires_auth(permission) decorator method
    @INPUTS
        permission: string permission (i.e. 'post:drink')
